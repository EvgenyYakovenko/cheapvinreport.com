<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('dashboard.pages.store') }}" id="pageForm">
                @csrf
    ...
    <script>
        ...
        // Auto-generate slug from main language title
        function generateSlug() {
            if (!slugAutoGenerate) return;

            const mainTitleInput = document.getElementById(`title_${defaultLocale}`);
            const slugField = document.getElementById('slug');

            if (mainTitleInput && mainTitleInput.value && slugField && slugField.readOnly) {
                let slug = transliterate(mainTitleInput.value)
                    .toLowerCase()
                    .trim()
                    .replace(/[^\w\s-]/g, '')
                    .replace(/[\s_-]+/g, '-')
        ...
        // Auto-generate slug when entering main language title
        document.addEventListener('input', function(e) {
            if (e.target && e.target.id === `title_${defaultLocale}`) {
                generateSlug();
            }
        });
