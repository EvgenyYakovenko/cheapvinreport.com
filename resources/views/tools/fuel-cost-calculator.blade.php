{{-- STAGING: Free tool — Fuel Cost Calculator (pure client-side JS). --}}
@include('header')
        ...
        <div class="container mx-auto px-4">
            <div class="max-w-5xl mx-auto">
                <nav class="text-sm text-gray-500 mb-4" aria-label="Breadcrumb">
                    ...
                    <span class="mx-2">/</span>
                    <span class="text-gray-700">Fuel Cost Calculator</span>
                </nav>

                <h1 class="text-3xl lg:text-4xl font-bold mb-3 text-gray-900">Free Fuel Cost Calculator</h1>
                <p class="text-lg text-gray-700 mb-8 max-w-3xl">
                    Work out the fuel cost of a trip — or a whole year of driving — from distance,
                    fuel economy (MPG) and the price of gas. Updates as you type.
                </p>
                ...
                <div class="grid lg:grid-cols-5 gap-6 max-w-4xl">
                    <div class="lg:col-span-3 bg-white rounded-xl border border-gray-200 shadow-sm p-5 lg:p-6 space-y-4">
                        <div>
                            <label for="distance" class="block text-sm font-semibold text-gray-700 mb-1">Distance (miles)</label>
                            <input id="distance" type="number" min="0" step="1" value="1000"
                        ...
                        <div class="grid sm:grid-cols-2 gap-4">
                            <div>
                                <label for="mpg" class="block text-sm font-semibold text-gray-700 mb-1">Fuel economy (MPG)</label>
                                <input id="mpg" type="number" min="1" step="0.1" value="28"
                                       class="w-full px-4 py-3 rounded-lg border-2 border-gray-300 focus:border-primary-500 focus:ring-2 focus:ring-primary-200 text-gray-900 outline-none">
                            </div>
                            <div>
                                <label for="price" class="block text-sm font-semibold text-gray-700 mb-1">Gas price ($/gallon)</label>
                                <input id="price" type="number" min="0" step="0.01" value="3.50"
                                       class="w-full px-4 py-3 rounded-lg border-2 border-gray-300 focus:border-primary-500 focus:ring-2 focus:ring-primary-200 text-gray-900 outline-none">
                            </div>
                        </div>
                        <label class="flex items-center gap-2 text-sm text-gray-700">
                            <input id="roundtrip" type="checkbox" class="rounded border-gray-300"> Round trip (double the distance)
                        </label>
                    </div>

                    <div class="lg:col-span-2 bg-primary-600 text-white rounded-xl shadow-sm p-6 flex flex-col justify-center">
                        <div class="text-primary-100 text-sm mb-1">Total fuel cost</div>
                        <div id="total" class="text-4xl font-bold mb-4">$0</div>
                        <dl class="text-sm space-y-1 text-primary-50">
                            <div class="flex justify-between"><dt>Gallons used</dt><dd id="gallons">0</dd></div>
                            <div class="flex justify-between"><dt>Cost per mile</dt><dd id="perMile">$0</dd></div>
                        </dl>
                    </div>
                </div>
            </div>
            ...
            <div class="max-w-5xl mx-auto grid md:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="text-gray-600 leading-relaxed">
                        Gallons used = distance ÷ MPG. Total cost = gallons × price per gallon. For a
                        round trip we double the distance. Cost per mile is the total divided by the
                        miles driven — a handy figure for comparing two cars.
                    </p>
                </div>
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 mb-3">Use it for</h2>
                    <ul class="text-gray-600 leading-relaxed space-y-2 list-disc pl-5">
                        <li>A single road trip's gas budget</li>
                        <li>Yearly fuel cost (enter miles per year)</li>
                        <li>Comparing running costs of two vehicles</li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="container mx-auto px-4">
            <div class="max-w-5xl mx-auto bg-white rounded-xl border border-gray-200 p-5">
                ...
                <p class="text-gray-600 text-sm leading-relaxed">
                    Standard fuel-cost arithmetic (distance ÷ MPG × price). For official fuel-economy
                    ratings by model, see the U.S. DOE/EPA site
                    <a href="https://www.fueleconomy.gov/" target="_blank" rel="noopener nofollow" class="text-primary-600 underline">fueleconomy.gov</a>.
    ...
    <section class="py-12 bg-white border-t border-gray-100">
        <div class="container mx-auto px-4">
            <div class="max-w-5xl mx-auto bg-primary-50 border border-primary-100 rounded-xl p-6 lg:p-8 text-center">
                <h2 class="text-2xl font-bold text-gray-900 mb-2">Budgeting a used car? Check its past too</h2>
...
(function () {
    ...
    function calc() {
        let dist = num(d.value);
        if (rt.checked) dist *= 2;
        const economy = num(mpg.value);
        const gas = num(price.value);
        const gallons = economy > 0 ? dist / economy : 0;
        const cost = gallons * gas;
        total.textContent = usd(cost);
        gallonsEl.textContent = gallons.toLocaleString('en-US', {maximumFractionDigits: 1});
        perMileEl.textContent = dist > 0 ? usd(cost / dist) : '$0.00';
    }

    [d, mpg, price].forEach(function (e) { e.addEventListener('input', calc); });
    rt.addEventListener('change', calc);
    calc();
})();
</script>
