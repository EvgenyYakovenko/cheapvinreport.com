{{-- STAGING: Free tool — Car Affordability Calculator. i18n lang/toolpages.affordability + calc. --}}
@include('header')
        ...
        <div class="container mx-auto px-4">
            <div class="max-w-5xl mx-auto">
                <nav class="text-sm text-gray-500 mb-4" aria-label="Breadcrumb">
                    ...
                    <span class="text-gray-700">{{ __('tools.items.car-affordability-calculator.label') }}</span>
                </nav>
                <h1 class="text-3xl lg:text-4xl font-bold mb-3 text-gray-900">{{ __('tools.items.car-affordability-calculator.title') }}</h1>
                <p class="text-lg text-gray-700 mb-8 max-w-3xl">{{ __('toolpages.affordability.lead') }}</p>
                ...
                <div class="grid lg:grid-cols-5 gap-6 max-w-4xl">
                    <div class="lg:col-span-3 bg-white rounded-xl border border-gray-200 shadow-sm p-5 lg:p-6 space-y-4">
                        <div><label for="budget" class="block text-sm font-semibold text-gray-700 mb-1">{{ __('toolpages.affordability.l_budget') }}</label>
                            <input id="budget" type="number" min="0" step="10" value="400" class="w-full px-4 py-3 rounded-lg border-2 border-gray-300 focus:border-primary-500 focus:ring-2 focus:ring-primary-200 text-gray-900 outline-none"></div>
                    ...
                    <div class="lg:col-span-2 bg-gray-900 text-white rounded-xl shadow-sm p-6 flex flex-col justify-center">
                        <div class="text-gray-400 text-sm mb-1">{{ __('toolpages.affordability.r_title') }}</div>
                        <div id="price" class="text-4xl font-bold mb-4">$0</div>
                        <dl class="text-sm space-y-1 text-gray-300">
                            <div class="flex justify-between"><dt>{{ __('toolpages.affordability.r_loan') }}</dt><dd id="loan">$0</dd></div>
                            <div class="flex justify-between"><dt>{{ __('toolpages.affordability.r_cash') }}</dt><dd id="cash">$0</dd></div>
                        </dl>
                    </div>
                </div>
            </div>
    ...
    <section class="py-10 bg-white border-t border-gray-100">
        <div class="container mx-auto px-4">
            <div class="max-w-5xl mx-auto grid md:grid-cols-2 gap-6">
                <div><h2 class="text-2xl font-bold text-gray-900 mb-3">{{ __('toolpages.calc.how') }}</h2><p class="text-gray-600 leading-relaxed">{{ __('toolpages.affordability.m1_d') }}</p></div>
                <div><h2 class="text-2xl font-bold text-gray-900 mb-3">{{ __('toolpages.calc.gtk') }}</h2>
                    <ul class="text-gray-600 leading-relaxed space-y-2 list-disc pl-5"><li>{{ __('toolpages.affordability.m2_li1') }}</li><li>{{ __('toolpages.affordability.m2_li2') }}</li><li>{{ __('toolpages.affordability.m2_li3') }}</li></ul>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="py-8">
        <div class="container mx-auto px-4">
            <div class="max-w-5xl mx-auto bg-white rounded-xl border border-gray-200 p-5">
                ...
                <p class="text-gray-600 text-sm leading-relaxed">{{ __('toolpages.affordability.source_d') }}
    ...
    <section class="py-12 bg-white border-t border-gray-100">
        <div class="container mx-auto px-4">
            <div class="max-w-5xl mx-auto bg-acc-50 border border-acc-100 rounded-xl p-6 lg:p-8 text-center">
                <h2 class="text-2xl font-bold text-gray-900 mb-2">{{ __('toolpages.affordability.cta_h') }}</h2>
                <p class="text-gray-600 mb-5 max-w-2xl mx-auto">{{ __('toolpages.affordability.cta_d') }}</p>
...
(function () {
    ...
    function calc(){
        const pay=num(el.budget.value), down=num(el.down.value), trade=num(el.trade.value), r=num(el.apr.value)/100/12, n=Math.max(1,Math.round(num(el.term.value))||1);
        let loan = r===0 ? pay*n : pay*(1-Math.pow(1+r,-n))/r; const cash=down+trade;
